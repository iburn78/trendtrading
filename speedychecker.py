...
class speedychecker():
    def __init__(self): 
        ...
        self.target_data = get_target_data(code_to_test_yf, start_date, end_date)
    ...
    def run_(self):
        ...
        sm_print(f'rate: {format(self.stat["t_rr"], ".4f")} % - latest price reflected')
        if self.suspended: 
    ...
    def speedy_logic(self, cur_price, t):
        if len(self.mb) == 0:
            ...
            self.price_condition = cur_price
        else: 
            l = self.mb.iloc[-1] 
            if l.vol == 0: # all sold situation
                if self.price_condition > cur_price:
                ...
                if self.suspended:
                    if rr >= self.bounds_table.at['LB', int(l.ninv)]:
                        ...
                        self.stat['c_val'] = round(cur_price*(l.vol)*(1-TAX_RATE-FEE_RATE))
                        self.stat['t_rr'] = round((self.stat['cash']+self.stat['c_val'])/START_CASH*100-100, 4)
                        self.suspended = False
                    return
    ...
    def sell(self, cur_price, pvol, t=''): 
        ...
        self.stat['t_inv'] = 0
        self.stat['c_val'] = 0
        self.stat['t_rr'] = round(self.stat['cash']/START_CASH*100-100, 4)
